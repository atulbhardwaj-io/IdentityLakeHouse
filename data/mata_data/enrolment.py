...
DEFAULT_INPUT = Path(
    "data/api_data_aadhar_enrolment/api_data_aadhar_enrolment/api_data_aadhar_enrolment_combined.csv"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Add audit metadata columns to the enrolment combined CSV."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="Path to the enrolment CSV file.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def build_batch_id(batch_id: str | None) -> str:
    if batch_id:
        return batch_id
    return datetime.now(timezone.utc).strftime("enrolment_%Y%m%dT%H%M%SZ")
