...
DEFAULT_INPUT = Path(
    "data/api_data_aadhar_demographic/api_data_aadhar_demographic/api_data_aadhar_demographic_combined.csv"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Add audit metadata columns to the demographic combined CSV."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def build_batch_id(batch_id: str | None) -> str:
    if batch_id:
        return batch_id
    return datetime.now(timezone.utc).strftime("demographic_%Y%m%dT%H%M%SZ")
