...
# ============================================================
# STEP 2: AADHAAR SILVER TRANSFORM
# ============================================================

if ($RunAadhaarTransform) {

    Write-Host ""
    Write-Host "[2/2] Aadhaar Silver transform (test path)..." `
        -ForegroundColor Yellow


    # Build transform command as ONE SINGLE LINE

    $transformCmd = "/opt/spark/bin/spark-submit --master $MasterUrl --packages io.delta:delta-spark_2.12:3.0.0 --conf spark.jars.ivy=/tmp/.ivy2 --conf spark.sql.extensions=io.delta.sql.DeltaSparkSessionExtension --conf spark.sql.catalog.spark_catalog=org.apache.spark.sql.delta.catalog.DeltaCatalog --conf spark.sql.catalogImplementation=hive --conf spark.sql.warehouse.dir=$WarehouseDir --conf 'spark.hadoop.javax.jdo.option.ConnectionURL=$MetastoreUrl' /app/scripts/silver_layer/transform_aadhaar_voter_link_silver.py --input-path /app/scripts/silver_layer/aadhaar_voter_link_raw_valid_test --output-path /app/scripts/silver_layer/aadhaar_voter_link_raw_valid_test --main-path /app/scripts/silver_layer/aadhaar_voter_link_raw_valid --run-id $RunId"


    # Optional promote flag
    ...
    if ($Promote) {
        ...
        Write-Host ""
        Write-Host "[WARN] Promote is ON." `
            -ForegroundColor Red

        Write-Host "       Main Silver table will be overwritten after transform." `
            -ForegroundColor Red
        ...
        Write-Host ""
        Write-Host "[INFO] Test mode." `
            -ForegroundColor Green

        Write-Host "       Main Silver table will NOT be changed." `
            -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "[SKIP] Aadhaar transform skipped." `
        -ForegroundColor DarkYellow

    Write-Host "       Use -RunAadhaarTransform to run it." `
        -ForegroundColor DarkYellow
